...
public class Highscores extends CCLayer implements KeyHandler{
	...
	private CCNode buildTables(float colWidth) {
		...
		CCNode easy = buildTable(Game.Difficulty.Easy, colWidth * 1);
		CCNode normal = buildTable(Game.Difficulty.Normal, colWidth * 3);
		CCNode hard = buildTable(Game.Difficulty.Hard, colWidth * 2);
	...
	private CCNode buildTable(Game.Difficulty difficulty, float x) {
		CCNode result = CCNode.node();
		...
		CCSprite header = CCSprite.sprite( 
				difficulty == Game.Difficulty.Easy ?
						"easy.png":
							(difficulty == Game.Difficulty.Normal ? "normal.png" : "hard.png"));
		...
		for (Long score : scores) {
			...
			TextEntry lbl = new TextEntry(TextEntry.TextBoxType.HighscoreEntry, str);
